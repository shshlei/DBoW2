...
template <class TDescriptor, class F>
inline std::size_t TemplatedDatabase<TDescriptor, F>::size() const
{
    return m_nentries;
}
                                             ...
                                             const std::string &name) const
{
    ...
    for (dit = m_dfile.begin(); dit != m_dfile.end(); ++dit)
    {
        ...
        for (drit = dit->begin(); drit != dit->end(); ++drit)
        {
            ...
            const std::vector<std::size_t> &features = drit->second;

            // save info of last_nid
            fs << "{";
            fs << "nodeId" << (int)nid;
            // msvc++ 2010 with opencv 2.3.1 does not allow
            // FileStorage::operator<< with vectors of std::size_t
            fs << "features"
        ...
        for (EntryId eid = 0; eid < fn.size(); ++eid)
        {
            ...
            for (std::size_t i = 0; i < fe.size(); ++i)
            {
                ...
                dit = m_dfile[eid].insert(
                    m_dfile[eid].end(),
                    make_pair(nid, std::vector<std::size_t>()));
