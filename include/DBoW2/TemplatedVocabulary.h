    ...
    int current_level)
{
    ...
    else
    {
        ...
        while (goon)
        {
            ...
            groups.clear();
            groups.resize(clusters.size(), std::vector<std::size_t>());
            ...
            else
            {
                ...
                goon = false;
                for (std::size_t i = 0; i < current_association.size(); i++)
                {
...
void TemplatedVocabulary<TDescriptor, F>::setNodeWeights(
    const std::vector<std::vector<TDescriptor> > &training_features)
{
    const std::size_t NWords = m_words.size();
    const std::size_t NDocs = training_features.size();

    if (m_weighting == TF || m_weighting == BINARY)
    {
        // idf part must be 1 always
        for (std::size_t i = 0; i < NWords; i++) m_words[i]->weight = 1;
    }
    else if (m_weighting == IDF || m_weighting == TF_IDF)
    {
        ...
        std::vector<std::size_t> Ni(NWords, 0);
...
template <class TDescriptor, class F>
inline std::size_t TemplatedVocabulary<TDescriptor, F>::size() const
{
    return m_words.size();
}
...
template <class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor, F>::saveToBinFile(
    const std::string &filename) const
{
    std::fstream f;
    f.open(filename.c_str(), std::ios_base::out);
    ...
    std::size_t nbiter = m_nodes.size();
    // nbiter=2; //for test
    unsigned char one = 1;
    unsigned char zero = 0;
    //    unsigned char tab[10]={1,2,4,6,10,100,200,201,202,255}; f.write((const
    //    char*)tab,10); //for test
...
template <class TDescriptor, class F>
bool TemplatedVocabulary<TDescriptor, F>::loadFromBinFile(
    const std::string &filename)
{
    std::ifstream f;
    f.open(filename.c_str());
    ...
    while ((!f.eof()) && (m_nodes.size() < (std::size_t)expected_nodes))
    {
        int nid = m_nodes.size();
        m_nodes.resize(m_nodes.size() + 1);
        m_nodes[nid].id = nid;
